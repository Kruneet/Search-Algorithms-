...
public class astar
{
    ...
    public static void main(String args[])
    {
        ...
        obj.i="123456789abcd0ef";
        obj.n.state=obj.i;
        obj.q.push(obj.n);
        //calling a function to solve by using manhattan distance.
        obj.manhattanstar(obj.q.peek(),0);
        node f= obj.q.peek();
        ...
        if(f.state.equalsIgnoreCase("123456789abcdef0"))
        {
            ...
            System.out.println("\nFailed by Manhattan distance heuristic at state- "+f.state);
        System.out.println("\nMemory used: "+ (runtime.totalMemory()-runtime.freeMemory())/(1024)+"KB");
        obj.endtime=(System.nanoTime() - obj.starttime );
        System.out.println("Time taken: " + obj.endtime+" nanoseconds");
        obj.starttime=System.nanoTime();
        //referesing the global variables for misplaced tile heuristic.
        obj.q= new Stack<node>();
        obj.map= new HashMap<String,Integer>();
        obj.q.push(obj.n);
        //calling a function to solve by displaced tile heuristic
        obj.displacedtile(obj.q.peek(),0);
        f= obj.q.peek();
        //check to see if goal state reached
        if(f.state.equalsIgnoreCase("123456789abcdef0"))
        {
            System.out.println("\nSolution by displaced tiles");
            System.out.println("\nMoves: ");
            int i=0;
            while(i<obj.q.size())
            {
    ...
    // Finds the goal state using manhattan distance heuristic
    void manhattanstar(node n,int cost)
    {
        ...
        if((!c.equalsIgnoreCase(goal))&& mem < 100)     //goal state and memory sanity check
        {
            ...
            for(int i=0;i<l;i++)
            {
                if(!map.containsKey(u1[i]))//check for repeated states.
                {
                    ...
                    manhattanstar(q.peek(),++cost);//recursive call
                    node t=q.peek();
                    String t1=t.state;
    ...
    void displacedtile(node n,int cost)
    {
        String goal="123456789abcdef0";
        String c= n.state;
        Runtime runtime = Runtime.getRuntime();
        ...
        if((!c.equalsIgnoreCase(goal))&& mem < 200)//goal and memory sanity check
        {
            ...
            {
                u1[l]= upPuzzle(c);
                u2[l]=misplaced(u1[l++])+cost;//f(n)=h(n)+g(n)
            }
            if(a<12)
            {
                u1[l]= downPuzzle(c);
            ...
            for(int i=0;i<l;i++)
            {
                if(!map.containsKey(u1[i]))//checking for repeated states
                {
                    node z= new node(u1[i]);
                    q.push(z);//adding to frontier
                    map.put(u1[i],i);//adding to explored nodes
                    displacedtile(q.peek(),++cost);//recursive call
                    node t=q.peek();
                    String t1=t.state;
    ...
    //Calculates the no of misplaced tiles at a particular state
    int misplaced(String s)
    {
        int l=s.length();
        int count=0;
        char c,f;
